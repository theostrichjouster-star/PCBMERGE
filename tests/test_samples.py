...
def merged(samples, tmp_path_factory):
    ...
    plan = MergePlan(output="combo", designs=specs)
...
def test_a_source_board_survives_as_a_rigid_translation(merged, samples):
    ...
    moved = {e.get("name")[len("MAX3_"):]: (float(e.get("x")), float(e.get("y")))
             for e in merged_doc.elements() if e.get("name").startswith("MAX3_")}
...
def test_rotations_survive_the_move(merged, samples):
    ...
    for element in merged_doc.elements():
        ...
        if name.startswith("MAX3_"):
            assert element.get("rot", "") == source[name[len("MAX3_"):]]
